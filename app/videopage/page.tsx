...
const handleDeleteClick = async (id: string) => {
  try {
    ...
    if (response.ok) {
      setVideos(videos.filter(video => video._id !== id)); // This updates the local UI
  ...
  // Refresh videos when component mounts or when navigating back from upload
  useEffect(() => {
    if (status === 'authenticated') {
      const fetchVideos = async () => {
        try {
  ...
  }, [status]); // Dependency on status to run on mount and when status changes
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="container mx-auto px-4 py-12">
        ...
        {session ? (
          ...
          <VideoFeed videos={videos} onVideoDelete={handleDeleteClick} />
        ) : (
